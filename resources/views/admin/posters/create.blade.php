...
<head>
    ...
    <title>Adding Asset - Admin Core</title>
...
<body class="flex items-center justify-center min-h-screen bg-black p-4">
    <div class="w-full max-w-lg bg-[#0a0a0a] rounded-xl border border-gray-800 p-8 shadow-2xl">
        <div class="flex justify-between items-center mb-8 border-b border-gray-800 pb-4">
            <h1 class="text-xl font-bold text-cyan-400">Initialize New Asset</h1>
            <a href="{{ route('admin.dashboard') }}" class="text-xs text-gray-500 hover:text-white uppercase tracking-wider">Cancel</a>
        </div>

        <form action="{{ route('admin.posters.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                <label class="block text-xs uppercase text-gray-500 mb-2">Asset Title</label>
                <input type="text" name="title" required class="w-full p-3 rounded input-field" placeholder="e.g. Cyber City">
            </div>
            
            <div>
                <label class="block text-xs uppercase text-gray-500 mb-2">Category Tag</label>
                <input type="text" name="category" required class="w-full p-3 rounded input-field" placeholder="e.g. Sci-Fi">
            </div>

            <div>
                <label class="block text-xs uppercase text-gray-500 mb-2">Visual Data Source (URL)</label>
                <input type="url" name="image" required class="w-full p-3 rounded input-field" placeholder="https://example.com/image.jpg">
            </div>

            <button type="submit" class="w-full py-4 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold rounded shadow-lg transform hover:-translate-y-1 transition text-sm uppercase tracking-widest">
                Compile & Save
            </button>
        </form>
    </div>
